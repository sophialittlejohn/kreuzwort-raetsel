...
export const doesAnswerFitInGrid = (
  answer: string,
  intersectingItems: Grid
) => {
  // find index of LIC (letter in common)
  const indexLIC = answer
    .toUpperCase()
    .split("")
    .findIndex((letter) => letter === intersectingItems[0].value);
  ...
  const firstHalfFits =
    intersectingItems[0].coordinates.y - (preLIC.length + 1) > 0;
  console.log("does it fit before?", firstHalfFits);
  // Does second half of word fit?
  const secondHalfFits =
    intersectingItems[0].coordinates.y - postLIC.length <= COLUMNS_COUNT;
